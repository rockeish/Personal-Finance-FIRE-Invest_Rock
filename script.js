const AtlasApp = {
    ...
    config: {
        ...
        NECESSARY_CATEGORIES: ["🏡 Housing", "💡 Utilities", "🛒 Food", "🚗 Transportation", "⚕️ Healthcare"]
    },
    ...
    cacheDOMElements() {
        ...
        this.elements.expensesContainer = document.getElementById('expensesContainer');
        this.elements.resultsSection = document.getElementById('results');
        this.elements.budgetTypeRadios = document.querySelectorAll('input[name="budgetFor"]');
    ...
    setupEventListeners() {
        ...
        if (this.elements.budgetForm) {
            this.elements.budgetTypeRadios.forEach(radio => radio.addEventListener('change', () => this.toggleCoupleMode()));
    ...
    toggleCoupleMode() {
        ...
        document.querySelectorAll('.partner-input').forEach(el => el.classList.toggle('hidden', !isCouple));
    },

    calculateBudget(data) {
        const isCouple = data.budgetFor === 'couple';
        const results = {
            user: { income: +data.userIncome || 0, expenses: 0, necessary: 0, discretionary: 0 },
            partner: { income: +data.partnerIncome || 0, expenses: 0, necessary: 0, discretionary: 0 },
            categoryTotals: {},
        };
    ...
    renderInsightsCard(results) {
        let insightsHTML = '';
        if (results.surplus > 0) insightsHTML += `<p>Great job! You have a surplus. Consider allocating more to savings or investments.</p>`;
        else if (results.surplus < 0) insightsHTML += `<p>You're in a deficit. Review your discretionary spending to find areas to cut back.</p>`;
        else insightsHTML += `<p>Your budget is balanced. Aim to create a surplus for unexpected costs.</p>`;
        ...
        this.state.budgetChart = new Chart(this.elements.canvas, {
            ...
            data: {
                ...
                datasets: [{
                    ...
                    backgroundColor: ['#4A90E2', '#50E3C2', '#F5A623', '#F8E71C', '#BD10E0', '#7ED321', '#9013FE', '#B8E986', '#417505', '#E94E77'],
                    borderColor: '#fff',
                    borderWidth: 2,
                    hoverOffset: 4
                }]
    ...
    saveTransactions(data) {
        let transactions = JSON.parse(localStorage.getItem('atlasTransactions')) || [];
        ...
        if (+data.userIncome > 0) {
            newTransactions.push({ description: 'Your Income', category: 'Income', amount: +data.userIncome });
        }
        if (+data.partnerIncome > 0) {
             newTransactions.push({ description: "Partner's Income", category: 'Income', amount: +data.partnerIncome });
        }

        for (const key in data) {
            const amount = +data[key] || 0;
            if (amount > 0 && !['budgetFor', 'userIncome', 'partnerIncome'].includes(key)) {
                const element = this.elements.budgetForm.elements[key];
                const description = element.closest('.input-grid').querySelector('label').textContent;
    ...
    renderTransactions() {
        ...
        const transactions = JSON.parse(localStorage.getItem('atlasTransactions')) || [];
        this.elements.transactionList.innerHTML = '';

        if (transactions.length === 0) {
            this.elements.transactionList.innerHTML = '<li class="transaction-item"><div class="transaction-info"><div class="description">No recent transactions.</div></div></li>';
            return;
        }
    ...
    loadData() {
        ...
        try {
            ...
            if (hasValues) {
                this.elements.budgetForm.dispatchEvent(new Event('submit', { cancelable: true }));
            }
        } catch (e) {
