...
const initialState: Omit<FinanceState,
	"addTransactions" | "clearTransactions" | "addCategory" | "removeCategory" | "setCategoryPlanned" | "addCategoryRule" | "removeCategoryRule" | "applyRules" | "setTransactionCategory" | "setCurrentMonth" | "setBudgetAmount" | "setHolding" | "removeHolding" | "setTargetAllocation" | "setBalance" | "setSettings" | "exportState" | "importState" | "resetAll" | "getMonthlyTransactions" | "getCategoryActuals"
> = {
	...
	categories: {
		...
		Groceries: { planned: 400, rules: ["grocery", "supermarket", "whole\\s?foods", "trader\\s?joe"] },
		Utilities: { planned: 150, rules: ["utility", "electric", "water", "internet", "cable"] },
		Fun: { planned: 200, rules: ["netflix", "spotify", "entertainment", "cinema", "movie", "hulu"] },
	},
	...
	settings: { currency: "USD", monthlyIncome: 6000, monthlyInvestContribution: 1000, withdrawalRatePercent: 4, enableRollovers: false },
};

function normalizeRow(row: Transaction): NormalizedTransaction | null {
	const rawDate = (row as any).date || (row as any).Date || (row as any).posted || (row as any).POSTED;
	...
	const description = String((row as any).description ?? (row as any).Description ?? (row as any).name ?? (row as any).memo ?? (row as any).Merchant ?? "").trim();
...
function coercePersisted(parsed: any): Partial<FinanceState> {
	...
	if (next.categories && typeof next.categories === "object") {
		for (const k of Object.keys(next.categories)) {
			next.categories[k] = {
				...
				rules: Array.isArray(next.categories[k]?.rules) ? next.categories[k].rules : [],
			};
		}
	}
	if (next.transactionCategories && typeof next.transactionCategories === "object") {
	...
	if (next.settings && typeof next.settings === "object") {
		next.settings = {
			...
			monthlyInvestContribution: Number(next.settings.monthlyInvestContribution ?? 0),
	...
	if (next.targetAllocation && typeof next.targetAllocation === "object") {
		...
		for (const k of Object.keys(next.targetAllocation)) m[k] = Number(next.targetAllocation[k] ?? 0);
...
export const useFinanceStore = create<FinanceState>((set, get) => {
	...
	return {
		...
		setCategoryPlanned: (name, amount) => set(() => {
			const categories = { ...get().categories };
			if (!categories[name]) categories[name] = { planned: 0, rules: [] };
		...
		addCategoryRule: (name, pattern) => set(() => {
			if (!name || !pattern) return get();
			const categories = { ...get().categories };
			if (!categories[name]) categories[name] = { planned: 0, rules: [] };
			if (!categories[name].rules.includes(pattern)) categories[name].rules.push(pattern);
			const next = { ...get(), categories } as FinanceState;
			persist(next);
			return next;
		}),
		removeCategoryRule: (name, pattern) => set(() => {
			const categories = { ...get().categories };
			if (categories[name]) categories[name].rules = categories[name].rules.filter((p) => p !== pattern);
			const next = { ...get(), categories } as FinanceState;
			persist(next);
			return next;
		}),
		applyRules: () => set(() => {
			const categories = get().categories;
			const catMap: Array<{ name: string; regs: RegExp[] }> = Object.entries(categories).map(([name, c]) => ({ name, regs: (c.rules || []).map((p) => { try { return new RegExp(p, "i"); } catch { return null as any; } }).filter(Boolean) as RegExp[] }));
			...
			for (const row of get().transactions) {
				...
				for (const { name, regs } of catMap) {
					if (regs.some((r) => r.test(n.description))) { map[n.key] = name; break; }
				}
			}
		...
		setTransactionCategory: (key, category) => set(() => {
			const map = { ...get().transactionCategories } as Record<string, string>;
			if (!category) delete map[key]; else map[key] = category;
			const next = { ...get(), transactionCategories: map } as FinanceState;
			persist(next);
			return next;
		}),
		setCurrentMonth: (ym) => set(() => {
		...
		setTargetAllocation: (symbol, percent) => set(() => {
			const next = { ...get(), targetAllocation: { ...get().targetAllocation, [symbol]: percent } } as FinanceState;
			persist(next);
			return next;
		}),
		...
		setSettings: (partial) => set(() => {
			const next = { ...get(), settings: { ...get().settings, ...partial } } as FinanceState;
			persist(next);
			return next;
		}),
