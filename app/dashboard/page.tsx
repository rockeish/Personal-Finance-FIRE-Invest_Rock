...
export default function DashboardPage() {
	const { balances, currentMonth, getCategoryActuals, settings } = useFinanceStore();
	const kpis = useMemo(() => {
		const actuals = getCategoryActuals(currentMonth);
		const totalSpent = Object.values(actuals).reduce((s, v) => s + v, 0);
		const savings = Math.max(0, (settings.monthlyIncome || 0) - totalSpent - (settings.monthlyInvestContribution || 0));
		return { totalSpent, savings };
	}, [currentMonth, getCategoryActuals, settings.monthlyIncome, settings.monthlyInvestContribution]);
