...
export default function BudgetPage() {
  ...
  const actuals = useMemo(
    () => getCategoryActuals(currentMonth),
    [getCategoryActuals, currentMonth]
  )
  const totalPlanned = Object.values(categories).reduce(
    (s, c) => s + (c.planned || 0),
    0
  )
              ...
              <tr key={name} className="border-b">
                ...
                <td className="text-right">
                  <input
                    ...
                    value={planned}
                    onChange={(e) =>
                      setCategoryPlanned(name, Number(e.target.value))
                    }
                  />
                </td>
                <td className="text-right">{actual.toLocaleString()}</td>
                <td className="align-middle">
                  <ProgressBar value={actual} max={planned} />
                </td>
              </tr>
            )
          })}
