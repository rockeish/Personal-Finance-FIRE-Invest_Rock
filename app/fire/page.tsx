			...
			<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
				<section className="rounded border p-4 space-y-3">
					...
					<label className="block text-sm">Withdrawal rate (%)
						<input className="mt-1 w-full border rounded px-2 py-1" type="number" value={settings.withdrawalRatePercent} onChange={(e) => setSettings({ withdrawalRatePercent: Number(e.target.value) })} />
					</label>
					<label className="block text-sm">Monthly investment contribution ($)
					...
					<p className="text-xs text-gray-500">Adjust budgeted expenses in <Link className="text-brand" href="/budget">Budget</Link> and target allocations in <Link className="text-brand" href="/investments">Investments</Link>.</p>
				</section>
